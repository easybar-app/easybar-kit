...
@dataclass
class AliasValue:
    """A documented value in a Lua type alias."""
    value: str
    description: str = ""
...
@dataclass
class AliasDoc:
    """Documentation for a Lua type alias."""
    name: str
    description: str
    type_expr: str = ""
...
@dataclass
class FieldDoc:
    """Documentation for a Lua class field."""
    name: str
    type_expr: str
    description: str
    optional: bool
...
def comment_text(line: str) -> str | None:
    """Extract text from a Lua documentation comment."""
...
def normalize_lua_literal(value: str) -> str:
    """Normalize a Lua literal for documentation."""
    value = value.strip()
...
def heading_id(name: str) -> str:
    """Build a stable Markdown heading identifier."""
    return f"`{name}`"
...
class LuaDocParserState:
    """Incremental state for parsing Lua documentation."""
    ...
    def consume(self, raw_line: str) -> None:
        """Consume one Lua API source line."""
    ...
    def consume_alias(self, line: str) -> bool:
        """Consume an alias declaration when present."""
    ...
    def consume_alias_value(self, line: str) -> bool:
        """Consume a value for the active alias."""
    ...
    def consume_class(self, line: str) -> bool:
        """Consume a class declaration when present."""
    ...
    def consume_field(self, line: str) -> bool:
        """Consume a field declaration when present."""
    ...
    def consume_param(self, line: str) -> bool:
        """Consume a parameter annotation when present."""
        param_match = re.match(r"---@param\s+(\S+)\s+(.+)", line)
        if not param_match:
            return False

        name = param_match.group(1)
        type_expr, description = split_type_and_description(
            param_match.group(2))
        optional = name.endswith("?")
        if optional:
            name = name[:-1]
    ...
    def consume_return(self, line: str) -> bool:
        """Consume a return annotation when present."""
        return_match = re.match(r"---@return\s+(.+)", line)
        if not return_match:
            return False

        type_expr, remainder = split_type_and_description(
    ...
    def consume_function(self, line: str) -> bool:
        """Consume a function declaration when present."""
    ...
    def capture_function_description(self) -> None:
        """Attach pending comments to the active function."""
    ...
    def pending_description(self) -> str:
        """Return pending comments as a description."""
        return " ".join(self.pending_comments).strip()

    def clear_pending_function_context(self) -> None:
        """Clear state for the active function declaration."""
        self.pending_comments = []
...
def parse_docs(
    text: str,
) -> tuple[dict[str, AliasDoc], dict[str, ClassDoc], dict[str, FunctionDoc]]:
    """Parse Lua API documentation from source lines."""
...
def add_inline_function_fields(
...
def function_from_field(name: str, field_doc: FieldDoc) -> FunctionDoc:
    """Convert a function-typed field into function documentation."""
...
def parse_fun_type(type_expr: str) -> tuple[list[ParamDoc], list[ReturnDoc]]:
    """Parse parameters and returns from a Lua function type."""
...
def split_top_level(text: str, delimiter: str) -> list[str]:
    """Split text outside nested type syntax."""
    parts: list[str] = []
    depth = 0
    current: list[str] = []
...
def write_index() -> None:
    """Write the Lua reference index page."""
...
1. Start with [Lua Widgets](../index.md) for the node-based mental model.
...
def render_alias(alias: AliasDoc, heading_level: int = 2) -> list[str]:
    ...
    if alias.type_expr:
        lines += [
            "| Value |",
            "| ----- |",
            f"| `{display_type(alias.type_expr)}` |",
            "",
        ]
    ...
    if alias.values:
        ...
        for item in alias.values:
            lines.append(
                f"| `{table_escape(item.value)}` | {table_escape(item.description)} |"
            )
        lines.append("")
...
def render_class(cls: ClassDoc, heading_level: int = 2) -> list[str]:
    ...
    if cls.fields:
        ...
        for item in cls.fields:
            ...
            lines.append(
                f"| {name} | `{display_type(item.type_expr)}` | "
                f"{table_escape(item.description)} |"
            )
        lines.append("")
...
def render_function(fn: FunctionDoc, heading_level: int = 2) -> list[str]:
    ...
    if fn.params:
        ...
        for param in fn.params:
            ...
            lines.append(
                f"| {name} | `{display_type(param.type_expr)}` | "
                f"{table_escape(param.description)} |"
            )
        lines.append("")
    ...
    if fn.returns:
        ...
        if any(ret.name or ret.description for ret in fn.returns):
            ...
            for ret in fn.returns:
                ...
                lines.append(
                    f"| `{display_type(ret.type_expr)}` | {name} | "
                    f"{table_escape(ret.description)} |"
                )
        else:
            for ret in fn.returns:
...
def write_functions(
    ...
    """Write grouped function reference pages."""
    preferred = [
        "EasyBar.add",
...
def write_node_kinds(
    aliases: dict[str, AliasDoc],
    classes: dict[str, ClassDoc],
) -> None:
    """Write node-kind reference documentation."""
    lines = [
        HEADER,
        "# Node Kinds",
        "",
...
def write_events(
    ...
    """Write event reference documentation."""
    names = [
        name
        for name in aliases
        if "Event" in name
    ...
    class_names = [
        name
        for name in classes
        if "Event" in name or name.endswith("EventData")
    ]
...
def write_properties(
    ...
    """Write property reference documentation."""
    property_aliases = [
        "EasyBarIconLike",
        "EasyBarLabelLike",
        "EasyBarPosition",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Directory that receives the generated Lua reference pages",
    )
    ...
    print(f"Generated Lua reference docs in {OUT}")
    return 0
