...
final class AppController {
  ...
  /// Product identity supplied by the frontend executable.
  private let identity: EasyBarApplicationIdentity
  /// Factory that constructs the concrete macOS frontend.
  private let surfaceFactory: EasyBarSurfaceFactory
  /// Explicit app-wide service graph bootstrapped during startup wiring.
  private let services: AppServices
  /// Public rendering facade shared with the concrete frontend.
  ...
  /// Concrete frontend currently presenting the shared widget runtime.
  private var surfaceController: (any EasyBarSurfaceController)?
  ...
  init(
    ...
    self.startupDiagnostics = AppStartupDiagnostics(
  ...
  func start() -> Bool {
    ...
    setupSurfaceController(menuFactory: menuFactory)
  ...
  func handlePostConfigReloadUI() {
    installWidgetEditorStub()
    surfaceController?.reloadLayout()
  ...
  private func configureLogging() {
    AppShellSupport.configureLogging(
      ...
      logFileName: identity.logFileName
    )
  }

  /// Creates and presents the concrete frontend selected by the executable.
  private func setupSurfaceController(menuFactory: EasyBarMenuFactory) {
    let context = EasyBarSurfaceContext(
      logger: logger.child("surface"),
      presentationModel: presentationModel,
      barContextMenuBuilder: { showDeveloperSection in
    ...
    let controller = surfaceFactory(context)
    controller.present()
  ...
  private func startBar() {
    runBarLifecycleTransition(finalState: .running) { [weak self] in
      guard let self else { return }
      await self.runtimeCoordinator.start()
      await MainActor.run { self.surfaceController?.present() }
    }
  }
  ...
  private func ensureShutdownTask() -> Task<Void, Never>? {
    ...
    logger.info("\(identity.processName) shutting down")
    signalHandler.stop()
